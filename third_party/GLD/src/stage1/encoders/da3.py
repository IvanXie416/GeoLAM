...
class DA3EncoderDirect(nn.Module):
    ...
    def forward(
        ...
        normalize_from_layer_patches: bool = True,
        ...
        Args:
            ...
            normalize_from_layer_patches: Whether 'from_layer' returns decoder-normalized
                patches. Disable it for the MAE decoder trained on raw mode='all' features.
        ...
        if mode == 'single':
            ...
            return self._forward_from_layer(
                ...
                total_view,
                grid_size=grid_size,
                normalize_patches=normalize_from_layer_patches,
            )
        else:
    ...
    def _forward_from_layer(
        ...
        Args:
            ...
            normalize_patches: Return [local, LayerNorm(current)] when True, or
                raw [local, current] patches when False.
        ...
        if start_layer_idx in self.OUT_LAYERS:
            ...
            start_patches = out_patches_norm if normalize_patches else x_flat[:, 1:]
            results[start_layer_idx] = (cls_raw, start_patches)
        ...
        for i, blk in enumerate(trans.blocks):
            ...
            if i in self.OUT_LAYERS:
                ...
                output_patches = out_final[:, 1:] if normalize_patches else out_raw[:, 1:]

                results[i] = (cls_raw, output_patches)
