...
class RAE_DA3(nn.Module):
    ...
    def decode(
        ...
        Args:
            feats: [(patches, cls)] for levels L0..L3. DPT expects layer-normalized
                   patches; the shipped MAE checkpoint expects raw mode='all' patches.
    ...
    def propagate_features(
        ...
        Args:
            ...
            normalize_patches: Return layer-normalized patches when True, or raw
                patches matching encode(mode='all') when False.

        Returns:  [layer_norm or raw patches, raw cls]
            List of (patches_4d, cls_3d) tuples for each level >= from_level
            patches_4d: (B, V, N, C)  — representation selected by normalize_patches
        ...
        raw_results = self.encoder.forward(
            ...
            normalize_from_layer_patches=normalize_patches,
        )
        
        # Reshape outputs for DPT decoder
        final_results = []
        for (cls_raw, output_patches) in raw_results:
            b_tot = cls_raw.shape[0]
            batch_size = b_tot // total_view
            
            n, c = output_patches.shape[1], output_patches.shape[2]
            patches_4d = output_patches.reshape(batch_size, total_view, n, c)
